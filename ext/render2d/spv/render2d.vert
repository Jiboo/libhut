...
layout(location = 0) out flat uvec4 out_box;
...
/*
  4 MSB of pos.x: round corner radius (8px per step)
  4 MSB of pos.y: round corner softness (1px per step)
...
void main() {
  const mat4 proj_view = ubo.proj * ubo.view;

  const uvec4 pos_box = in_i_pos_box_r16g16b16a16_uint & 0x0FFF;
  ...
  const float radius = (in_i_pos_box_r16g16b16a16_uint.x >> 12) * ubo.dpi_factor;
  ...
  out_box = uvec4(pos_box.x + softness2, pos_box.y + softness2, pos_box.z - softness2, pos_box.w - softness2);

  const uint indices[] = {0, 1, 2, 2, 1, 3};

  switch (indices[gl_VertexIndex]) {
    case 0: // top left
      gl_Position = proj_view * vec4(pos_box.x, pos_box.y, 0, 1);
      out_uv = vec2(uv_box.x, uv_box.y);
      out_col = out_params.w == GRADIENT_TR2BL ? col_mix : col_from;
    break;
    case 1: // bottom left
      gl_Position = proj_view * vec4(pos_box.x, pos_box.w, 0, 1);
    ...
    break;
    case 2: // top right
      gl_Position = proj_view * vec4(pos_box.z, pos_box.y, 0, 1);
    ...
    break;
    case 3: // bottom right
      gl_Position = proj_view * vec4(pos_box.z, pos_box.w, 0, 1);
